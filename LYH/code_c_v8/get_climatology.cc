...
#define DEFAULT_SAVE_PATH "/public/home/mcc20262029/lyh/code_c_v8/ERA5/Climatology/"
...
//GPU核函数，计算每个网格点的Climmean和P90
__global__ void compute_stats_kernel(const float * __restrict__ sst_data,double * __restrict__ Clim,double * __restrict__ P90,const int * __restrict__ d_slot_valid,size_t grid_size,int n_valid){

    unsigned long long gid = (unsigned long long)blockIdx.x * (unsigned long long)blockDim.x + (unsigned long long)threadIdx.x;
...
//从原始数据文件中读取SST数据，存储在buf指向的内存中，读取的字节数为nbytes
...
//写出最终的结果到HDF5文件中，包括经纬度数据、目标doy、Climmean和P90
void write_output_hdf5(const char *filename, double *lon, double *lat,int lon_num, int lat_num, int doy,double *Clim, double *P90) {
    ...
    hsize_t dims[1] = {1};
    hid_t space = H5Screate_simple(1, dims, NULL);
    hid_t dset = H5Dcreate2(file_id, "dayofyear", H5T_NATIVE_DOUBLE, space, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
...
int main(int argc, char **argv) {
    ...
    int dev_count = 0;
    hipError_t dev_err = hipGetDeviceCount(&dev_count);
    if (dev_err != hipSuccess || dev_count <= 0) {
        fprintf(stderr, "[rank %d/%d] hipGetDeviceCount failed: %s\n",
                mpi_rank, mpi_size, hipGetErrorString(dev_err));
        MPI_Abort(MPI_COMM_WORLD, 1);
    }
    int dev_id = local_rank % dev_count;
    hipError_t set_err = hipSetDevice(dev_id);
    if (set_err != hipSuccess) {
        fprintf(stderr, "[rank %d/%d] hipSetDevice(%d) failed: %s\n",
                mpi_rank, mpi_size, dev_id, hipGetErrorString(set_err));
        MPI_Abort(MPI_COMM_WORLD, 1);
    }

    //解析命令行参数，设置nc_path、save_path、start_doy和end_doy
    ...
    //初始化sst_data为NAN，表示无效值
    #pragma omp parallel for
    for (size_t i = 0; i < SAMPLE_TOTAL * grid_size; i++) {
        sst_data[i] = NAN;
    }

    //检查所有文件的存在性，存储在file_valid数组中，避免后续重复检查文件存在性
    ...
    //主循环，遍历每个目标doy，更新窗口数据，计算统计量，并写出结果
    for (int doy = start_doy; doy <= end_doy; doy++) {
        ...
        if (doy > start_doy && next_new_doy >= 1 && next_new_doy <= 365) {
            for (int yi = 0; yi < N_YEARS; yi++) {
                int slot = yi * N_OFFSETS + next_slot_oi;
                hipMemcpyAsync(d_sst_data + slot * grid_size,sst_data + slot * grid_size,flt_bytes, hipMemcpyHostToDevice, stream);
            }
            hipMemcpyAsync(d_slot_valid, slot_valid, SAMPLE_TOTAL * sizeof(int),hipMemcpyHostToDevice, stream);
        }

        compute_stats_kernel<<<grid_size, GPU_THREADS, 0, stream>>>(d_sst_data, d_Clim, d_P90, d_slot_valid, grid_size, n_valid);
