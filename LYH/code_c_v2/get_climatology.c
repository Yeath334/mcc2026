...
#define SAVE_PATH "/public/home/mcc20262029/lyh/code_c_v2/ERA5/Climatology/"
...
int main() {
    ...
    for (int doy = 152; doy <= 243; doy++) {
        ...
        for (int i = 0; i < lat_num * SAMPLE_TOTAL * lon_num; i++) {
            sst_temp[i] = NAN;
        }
            ...
            for (int offset = -DELTA_DAY; offset <= DELTA_DAY; offset++) {
                ...
                for(int i = 0; i < lat_num; i++){
                    ...
                    for (int j = 0; j < lon_num; j++) {
                        sst_temp[i * (lon_num * SAMPLE_TOTAL) + temp_idx * lon_num + j] = sst_full[i * lon_num + j];
                    }
                }
        ...
        for(int i = 0; i < lat_num; i++){
            ...
            for (int j = 0; j < lon_num; j++) {
                ...
                for (int k = 0; k < SAMPLE_TOTAL; k++) {
                    sample_values[k] = sst_temp[i * (lon_num * SAMPLE_TOTAL) + k * lon_num + j];
                }

                Clim[i * lon_num + j] = nanmean(sample_values, SAMPLE_TOTAL);
                P90[i * lon_num + j] = percentile90(sample_values, SAMPLE_TOTAL);
            }

            if ((i + 1) % 10 == 0 || i == lat_num - 1) {
