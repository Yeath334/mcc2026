...
#define SAVE_PATH "/public/home/mcc20262029/lyh/code_c_v3/ERA5/Climatology/"
    ...
    for (int doy = 152; doy <= 243; doy++) {
        ...
        for (int yr = START_YR; yr <= END_YR; yr++) {
            ...
            for (int offset = -DELTA_DAY; offset <= DELTA_DAY; offset++) {
                ...
                read_sst_full(input_file, sst_full);             //获得海温数据

                for(int i = 0; i < lat_num; i++){

                    size_t z = (size_t)i * (lon_num * SAMPLE_TOTAL);
                    size_t y_1 = (size_t)temp_idx * lon_num;
                    size_t y_2 = (size_t)i * lon_num;

                    memcpy(&sst_temp[z + y_1],&sst_full[y_2],(size_t)lon_num * sizeof(double));
                }
