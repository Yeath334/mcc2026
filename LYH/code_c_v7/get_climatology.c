...
#define SAVE_PATH "/public/home/mcc20262029/lyh/code_c_v7/ERA5/Climatology/"
...
int main(int argc, char **argv){
    ...
    int global_start_doy = 152;
    int global_end_doy   = 243;

    if (argc > 1 && argv[1] != NULL && strlen(argv[1]) > 0){
        nc_path = argv[1];
    }
    if (argc > 2){
        global_start_doy = atoi(argv[2]);
    }
    if (argc > 3){
        global_end_doy = atoi(argv[3]);
    }

    if (global_start_doy < 1) global_start_doy = 1;
    if (global_end_doy > 365) global_end_doy = 365;
    if (global_start_doy > global_end_doy){
        if (mpi_rank == 0){
            fprintf(stderr, "Invalid DOY range: %d - %d\n", global_start_doy, global_end_doy);
        }
        MPI_Finalize();
        return EXIT_FAILURE;
    }

    int total_doy = global_end_doy - global_start_doy + 1;
    int base = total_doy / mpi_size;
    int rem  = total_doy % mpi_size;
    int my_count = base + (mpi_rank < rem ? 1 : 0);
    int local_start_doy = global_start_doy + mpi_rank * base + (mpi_rank < rem ? mpi_rank : rem);
    int local_end_doy   = local_start_doy + my_count - 1;

    printf("[rank %d/%d] nc_path=%s\n", mpi_rank, mpi_size, nc_path);
    if (my_count > 0){
        printf("[rank %d/%d] global DOY %d-%d, local DOY %d-%d (%d days)\n",
               mpi_rank, mpi_size, global_start_doy, global_end_doy,
               local_start_doy, local_end_doy, my_count);
    }
    else{
        printf("[rank %d/%d] no DOY assigned.\n", mpi_rank, mpi_size);
        MPI_Finalize();
        return 0;
    }
    fflush(stdout);
    ...
    printf("[rank %d/%d] lon_num = %d, lat_num = %d\n", mpi_rank, mpi_size, lon_num, lat_num);
    fflush(stdout);
    ...
    if (stage_buf == NULL || stage_ok == NULL){
        fprintf(stderr, "[rank %d] Memory allocation failed for stage buffer.\n", mpi_rank);
    ...
    double sum_compute_write = 0.0;

    for (int doy = local_start_doy; doy <= local_end_doy; doy++){
        struct timeval doy_start, doy_end;
        gettimeofday(&doy_start, NULL);

        printf("[rank %d/%d] Calculating climatology for day %d...\n", mpi_rank, mpi_size, doy);
        fflush(stdout);

        if (doy == local_start_doy){
            load_initial_window(doy, sst_temp, sst_full_pool, lat_num, lon_num, slot_valid);
        }
        else{
            int overwrite_day_idx = (doy - (local_start_doy + 1)) % WINDOW_NUM;
            ...
            if (stage_ready && stage_slot == overwrite_day_idx){
                scatter_stage_to_slot(sst_temp, stage_buf, stage_ok, slot_valid,
                                      lat_num, lon_num, overwrite_day_idx);
                stage_ready = 0;
                stage_slot = -1;
            }
            else{
                load_new_to_slot(doy, overwrite_day_idx, sst_temp, sst_full_pool,
                                 lat_num, lon_num, slot_valid);
                stage_ready = 0;
                stage_slot = -1;
            }
        }
        ...
        int n_io = 0;
        if (doy < local_end_doy){
            n_io = IO_THREADS_PREFETCH;
        }

        int n_cmp = max_threads - n_io;
        if (n_cmp <= 0){
            fprintf(stderr, "[rank %d] n_cmp <= 0. Please set OMP_NUM_THREADS > IO_THREADS_PREFETCH.\n", mpi_rank);
            MPI_Abort(MPI_COMM_WORLD, EXIT_FAILURE);
        }
        ...
        if (doy < local_end_doy){
            next_overwrite_day_idx = (doy - local_start_doy) % WINDOW_NUM;
        ...
        #pragma omp parallel num_threads(max_threads)
        {
            ...
            if (tid < n_io){
                ...
                for (int i = i0; i < i1; i++){
                    ...
                    for (int jb = 0; jb < lon_num; jb += COMP_BLK){
                        int nb = COMP_BLK;
                        ...
                        for (int b = 0; b < nb; b++){
                            ...
                            Clim[out_idx] = (double)(sum / (float)count);
        ...
        sum_compute_write += doy_elapsed;

        printf("[rank %d/%d] Day %d elapsed wall time: %.2f seconds\n", mpi_rank, mpi_size, doy, doy_elapsed);
        printf("[rank %d/%d] ------------------------------------\n", mpi_rank, mpi_size);
    ...
    MPI_Reduce(&total_elapsed, &max_elapsed, 1, MPI_DOUBLE, MPI_MAX, 0, MPI_COMM_WORLD);

    printf("\n====================================\n");
    printf("[rank %d/%d] Local DOY range: %d - %d (%d days)\n",
           mpi_rank, mpi_size, local_start_doy, local_end_doy, my_count);
    printf("[rank %d/%d] Local elapsed wall time: %.2f seconds\n", mpi_rank, mpi_size, total_elapsed);
    printf("====================================\n");
    fflush(stdout);

    if (mpi_rank == 0){
        printf("\n====================================\n");
        printf("MPI total wall time, max over ranks: %.2f seconds\n", max_elapsed);
        printf("====================================\n");
        fflush(stdout);
    }
