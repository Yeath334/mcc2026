...
#define SAVE_PATH "/public/home/mcc20262029/yhy/mcc.c/ERA5/Climatology/"
...
void read_sst_full(const char *filename, double *sst_full) {
    ...
    H5_CHECK(H5Dread(dset, H5T_NATIVE_DOUBLE, H5S_ALL, H5S_ALL, H5P_DEFAULT, sst_full));
...
int main() {
    ...
    double *sst_temp = (double *)malloc(SAMPLE_TOTAL * grid_size * sizeof(double));
    double *sst_full = (double *)malloc(grid_size * sizeof(double));
    ...
    for (int doy = 152; doy <= 243; doy++) {
        ...
        for (int t = 0; t < SAMPLE_TOTAL; t++) {
            sst_temp[t * grid_size] = NAN;
        }
        ...
        for (int yr = START_YR; yr <= END_YR; yr++) {
            for (int offset = -DELTA_DAY; offset <= DELTA_DAY; offset++) {
                ...
                read_sst_full(input_file, sst_full);

                memcpy(&sst_temp[temp_idx * grid_size], sst_full,
        ...
        #pragma omp parallel
        {
            double *row_buf = (double *)malloc(lon_num * SAMPLE_TOTAL * sizeof(double));

            #pragma omp for schedule(dynamic)
            for (int i = 0; i < lat_num; i++) {
                int offset = i * lon_num;

                for (int k = 0; k < SAMPLE_TOTAL; k++) {
                    double *src = &sst_temp[k * grid_size + offset];
                    for (int j = 0; j < lon_num; j++) {
                        row_buf[j * SAMPLE_TOTAL + k] = src[j];
                    }
                }

                for (int j = 0; j < lon_num; j++) {
                    double *samples = &row_buf[j * SAMPLE_TOTAL];
                    Clim[offset + j] = nanmean(samples, SAMPLE_TOTAL);
                    P90[offset + j] = percentile90(samples, SAMPLE_TOTAL);
                }
            }
